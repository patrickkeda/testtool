...
from src.instruments.base import IPowerSupply
...
def reset_psu(ctx: Context, psu_id: str) -> bool:
    """
    重置电源
    ...
    try:
        ...
        ctx.log_info(f"电源 {psu_id} 重置完成")
        return True
        
    except Exception as e:
        ctx.log_error(f"电源 {psu_id} 重置失败: {e}")
        return False


def get_psu_status(ctx: Context, psu_id: str) -> Optional[str]:
    """
    获取电源状态
    ...
    try:
        ...
        status = f"连接:{connected}, 电压:{voltage:.3f}V, 电流:{current:.3f}A"
        ctx.log_info(f"电源 {psu_id} 状态: {status}")
        
        return status
        
    except Exception as e:
        ctx.log_error(f"获取电源 {psu_id} 状态失败: {e}")
        return None
